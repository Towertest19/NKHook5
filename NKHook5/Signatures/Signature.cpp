...
void Signatures::FindAll() {
	...
	/* TowerInfoScreen */
	pointerMap[Sigs::TowerInfoScreen_CCTOR] = Signatures::FindFirst(3,
		// TowerInfoScreen constructor - based on ScriptedScreen pattern
		"55 8B EC 6A ?? 68 ?? ?? ?? ?? 64 ?? ?? ?? ?? ?? 50 83 EC ?? A1 34 ?? ?? ?? 33 C5 ?? 45 ?? 56 57 50 8D ?? ?? ?? A3 ?? ?? ?? ?? 8B F1 8B ?? ?? ?? 45 ?? 8B ?? ?? 8B ?? ?? 50 51 6A",
		// Alternative pattern - similar to other screen constructors
		"55 8B EC 6A ?? 68 ?? ?? ?? ?? 64 ?? ?? ?? ?? ?? 50 81 EC ?? ?? ?? ?? A1 34 ?? ?? ?? 33 C5 ?? 45 ?? 53 56 57 50 8D ?? ?? ?? A3 ?? ?? ?? ??",
