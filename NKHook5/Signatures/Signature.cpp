...
#define VT_IDX(SIG_VT, IDX) (pointerMap.contains(SIG_VT) ? (pointerMap.at(SIG_VT) != nullptr ? ((*(void**)(((uintptr_t) pointerMap[SIG_VT]) + (sizeof(void*) * IDX))) != nullptr ? *(void**)(((uintptr_t) pointerMap[SIG_VT]) + (sizeof(void*) * IDX)) : nullptr) : nullptr) : nullptr)
...
void Signatures::FindAll() {
	...
	/* CBaseTower */
	pointerMap[Sigs::CBaseTower_IncrementUpgradePath] = Signatures::FindFirst(1,
		// BTD5 v4.7: updates the tower upgrade counter for the requested path and refreshes dependent tower state
		"55 8B EC 83 E4 ?? 51 53 56 57 8B 7D ?? 8B F1 8D 04 BD ?? ?? ?? ?? 89 BE 40 01 00 00"
	);
	/* CBloon */
	...
	pointerMap[Sigs::CTowerFactory_GetTowerName] = Signatures::FindFirst(3,
		//Kongregate - long pattern
		...
		// Steam - shorter pattern focusing on function prologue
		"55 8b ec 6a ?? 68 ?? ?? ?? ?? 64 a1 ?? ?? ?? ?? 50 83 ec ?? a1 ?? ?? ?? ?? 33 c5 89 45 ?? 53 56 57",
		// Relaxed - just the key bytes
		"55 8b ec 6a ?? 68 ?? ?? ?? ?? 64 a1 ?? ?? ?? ?? 50 83 ec ??"
	);
	pointerMap[Sigs::CTowerFactory_ApplyUpgrade] = Signatures::FindFirst(1,
		// BTD5 v4.7: applies one tower upgrade definition to a live tower, including tower and weapon upgrades
		"55 8B EC 6A ?? 68 ?? ?? ?? ?? 64 A1 ?? ?? ?? ?? 50 83 EC ?? A1 34 ?? ?? ?? 33 C5 89 45 ?? 53 56 57 50 8D 45 ?? 64 A3 ?? ?? ?? ?? 89 65 ?? 8B D9 8B 45"
	);
	pointerMap[Sigs::CTowerFactory_ApplyUpgrade] = Signatures::FindFirst(1,
		// BTD5 v4.7: applies one tower upgrade definition to a live tower, including tower and weapon upgrades
		"55 8B EC 6A ?? 68 ?? ?? ?? ?? 64 A1 ?? ?? ?? ?? 50 83 EC ?? A1 34 ?? ?? ?? 33 C5 89 45 ?? 53 56 57 50 8D 45 ?? 64 A3 ?? ?? ?? ?? 89 65 ?? 8B D9 8B 45"
	...
	/* JsonObject */
	pointerMap[Sigs::JsonObject_ReadBoolProperty] = Signatures::FindFirst(3,
		//Kong
		...
		// Steam - shorter pattern
		"55 8b ec 6a ?? 68 ?? ?? ?? ?? 64 a1 ?? ?? ?? ?? 50 83 ec ?? 56 a1 ?? ?? ?? ?? 33 c5 50 8d 45 ?? 64 a3 ?? ?? ?? ?? 8b f1 83 ec ??",
		// Relaxed - prologue only with bool init pattern
		"55 8b ec 6a ?? 68 ?? ?? ?? ?? 64 a1 ?? ?? ?? ?? 50 83 ec ?? 56"
	);
	pointerMap[Sigs::JsonObject_ReadIntProperty] = Signatures::FindFirst(3,
		//Kong
		"55 8b ec 6a ?? 68 ?? ?? ?? ?? 64 a1 ?? ?? ?? ?? 50 83 ec ?? 56 57 a1 ?? ?? ?? ?? 33 c5 50 8d 45 ?? 64 a3 ?? ?? ?? ?? 8b f1 8b 7d ?? c7 45 ?? ?? ?? ?? ?? 8d 4d",
		// Steam
		"55 8b ec 6a ?? 68 ?? ?? ?? ?? 64 a1 ?? ?? ?? ?? 50 83 ec ?? 56 57 a1 ?? ?? ?? ?? 33 c5 50 8d 45 ?? 64 a3 ?? ?? ?? ?? 8b f1 8b 7d",
		// Relaxed - prologue only
		"55 8b ec 6a ?? 68 ?? ?? ?? ?? 64 a1 ?? ?? ?? ?? 50 83 ec ?? 56 57"
	);
	/* Screens */
	...
	/* MonkeyLabScreen */
	pointerMap[Sigs::MonkeyLabScreen_CCTOR] = Signatures::FindFirst(2,
		// MonkeyLabScreen constructor - verified against BTD5-Win.exe.bak.
		// Anchor on the unique ScriptedScreen base-name construction for "MonkeyLabScreen"
		// but keep the exception prologue relaxed for Steam v4.7 minor compiler drift.
		"55 8B EC 6A ?? 68 ?? ?? ?? ?? 64 A1 ?? ?? ?? ?? 50 83 EC ?? 56 A1 ?? ?? ?? ?? 33 C5 50 8D 45 ?? 64 A3 ?? ?? ?? ?? 8B F1 ?? 75 ?? 83 EC ?? 8B CC ?? 65 ?? 6A 0F C7 41 14 0F 00 00 00 C7 41 10 00 00 00 00 68 18 A1 BE 00",
		"55 8B EC 6A FF 68 ?? ?? ?? ?? 64 A1 ?? ?? ?? ?? 50 83 EC 08 56 A1 ?? ?? ?? ?? 33 C5 50 8D 45 F4 64 A3 ?? ?? ?? ?? 8B F1 89 75 F0 83 EC 18 8B CC 89 65 EC 6A 0F C7 41 14 0F 00 00 00 C7 41 10 00 00 00 00 68 18 A1 BE 00"
	);
	/* SpecialtiesScreen */
	pointerMap[Sigs::SpecialtiesScreen_CCTOR] = Signatures::FindFirst(2,
		// SpecialtiesScreen constructor - verified against BTD5-Win.exe.bak.
		// Anchor on the unique ScriptedScreen base-name construction for "SpecialtiesScreen"
		// but keep the exception prologue relaxed for Steam v4.7 minor compiler drift.
		"55 8B EC 6A ?? 68 ?? ?? ?? ?? 64 A1 ?? ?? ?? ?? 50 83 EC ?? 56 A1 ?? ?? ?? ?? 33 C5 50 8D 45 ?? 64 A3 ?? ?? ?? ?? 8B F1 ?? 75 ?? 83 EC ?? 8B CC ?? 65 ?? 6A 11 C7 41 14 0F 00 00 00 C7 41 10 00 00 00 00 68 48 A1 BE 00",
