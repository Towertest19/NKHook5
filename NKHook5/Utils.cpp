...
size_t Utils::FindPattern(size_t rangeStart, size_t rangeEnd, std::string_view pattern) {
        ...
        if (sig.has_value()) {
                ...
                if (result.has_result()) {
                        ...
                        // NOTE: We intentionally do NOT call IsAddressExecutable() here.
                        // This overload is used by rttihelper to scan .rdata/.data for RTTI
                        // type-descriptor strings and vtable pointers, which live in
                        // non-executable data sections by design.  Applying an executability
                        // check here caused every h_rtti::get_vtable() call to fail silently,
                        // breaking all patches that depend on VTable-indexed function addresses.
