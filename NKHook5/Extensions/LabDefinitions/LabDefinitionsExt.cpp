...
void LabDefinitionsExt::UseJsonData(nlohmann::json content)
{
        ...
        try
        {
                ...
                if (def.labType >= 0)
                {
                        labTypeToIndex[def.labType] = definitions.size();
                }

                const std::string loadedName = def.name;
                const int loadedLabType = def.labType;
                const int loadedMaxLevel = def.maxLevel;
                definitions.emplace_back(std::move(def));

                Print(LogLevel::INFO, "Loaded LabDefinition: '%s' (LabType=%d) with max level %d",
                        loadedName.c_str(), loadedLabType, loadedMaxLevel);
        }
