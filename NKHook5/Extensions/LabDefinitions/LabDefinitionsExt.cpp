...
void LabDefinitionsExt::UseJsonData(nlohmann::json content)
{
	...
	try
	{
		...
		{
			// No upgrades array, keep default max level
			def.maxLevel = 13;
			Print(LogLevel::WARNING, "Lab '%s': no upgrades found, using default max level = %d", 
...
int LabDefinitionsExt::GetMaxLevel(const std::string& labName) const
{
	...
	Print(LogLevel::WARNING, "Lab '%s' not found, using default max level = 13", labName.c_str());
	return 13;
}
